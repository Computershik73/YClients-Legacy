...
/** Забывает выбранную услугу — её нельзя нести к другому сотруднику. */
- (void)forgetService {
    if (_serviceId == 0) {
        return;
    }

    // Пометка нужна правке: без неё сохранение вернуло бы прежний список
    // услуг записи, то есть ровно то, от чего мы сейчас отказались.
    _serviceChanged = YES;
    _serviceId = 0;
    _serviceTitle = nil;
}
...
- (void)loadServices {
    ...
    [[YCApi shared] loadServicesForStaff:forStaff completion:^(NSArray *services, NSString *error) {
         ...
         */
        /**
         * Если услуг у сотрудника нет — выбранная забывается.
         *
         * Иначе она оставалась от прежнего сотрудника и уходила в запрос:
         * в журнале это выглядело так, что приложение само написало
         * «Сотруднику 5950338 услуги не назначены», а через семь секунд
         * отправило серверу услугу другого сотрудника и получило отказ
         * «Сотрудник не оказывает выбранные услуги».
         *
         * Теперь запись просто уходит без услуги — с тех пор как услуга
         * стала необязательной, это законный исход, и он куда лучше
         * заведомо отвергаемого запроса.
         */
    ...
    switch (path.row) {
        ...
        case YCRowStaff: {
            YCStaffPickerController *picker =
                ...
                // У другого сотрудника свой набор услуг — перечитываем.
                [self loadServices];
            }];
