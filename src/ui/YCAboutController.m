...
static NSArray *YCAboutLinks(void) {
    return @[
        ...
        /**
         * Библиотека, на которой всё держится.
         *
         * Приложение — это оболочка вокруг cYclients: весь разговор
         * с api.yclients.com идёт через неё. Ссылка стоит здесь потому,
         * что чужую работу принято называть по имени, а не растворять
         * в своей; заодно это единственное место, откуда можно попасть
         * к исходникам.
         */
        @[ @"Библиотека cYclients",
           @"работа с API на C",
           @"https://github.com/igkuzm/cYclients" ]
    ];
}
