...
print('--- срок жизни: не должно быть ---')
ok &= check('Срок этой сборки истёк', 'текст об истечении', False)
ok &= check('работает до', 'строка со сроком', False)
ok &= check('clock.serverTime', 'ключ времени сервера', False)
ok &= check('EEE, dd MMM yyyy HH:mm:ss zzz', 'разбор заголовка Date', False)
